...
async function getAgentAssignments() {
  try {
    ...
    if (error.response) {
      ...
      // Check for specific error codes and handle them appropriately 
      if (response.status === 401) {
        // ... implement token refresh logic and retry the download
        logger.warn('Retrying getAgentAssignments after token refresh...');
        // ... your token refresh logic here ...
        // Make sure to update the 'accessToken' variable after refreshing
        return await getAgentAssignments(); 
      } else if (response.status === 404) { // File not found
        // You might want to create the file if it doesn't exist or handle this differently
        logger.warn('Agent assignments file not found, creating a new one...');
        await storeAgentAssignments([]); // Store an empty array initially
        return [];
      } else {
        throw new Error(errorMessage);
      }
